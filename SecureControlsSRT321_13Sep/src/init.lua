...
local function can_handle_secure_srt321_thermostat(opts, driver, device, cmd, ...)
    log.info("can_handle_secure_srt321_thermostat")
    for _, fingerprint in ipairs(SECURE_SRT_THERMOSTAT_FINGERPRINTS) do
        if device:id_match( fingerprint.manufacturerId, fingerprint.productType, fingerprint.productId) then
            log.info("can_handle_secure_srt321_thermostat: hit")
            return true
        end
    end

    log.info("can_handle_secure_srt321_thermostat: miss")
    return false
end
...
local function thermostat_mode_report_handler(self, device, cmd)
  ...
  local mode = 0 -- cmd.args.mode

  log.info("thermostat_mode_report_handler mode: ", mode)

  if mode == 0 then  -- OFF (IDLE)
    log.info("thermostat_mode_report_handler - IDLE")
    event = capabilities.thermostatMode.thermostatMode.off()
  elseif mode == 1 then  -- HEAT (HEATING)
    log.info("thermostat_mode_report_handler - HEATING")
    event = capabilities.thermostatMode.thermostatMode.heat()
  else
    log.info("thermostat_mode_report_handler - UNKNOWN -> OFF (IDLE)")
    event = capabilities.thermostatMode.thermostatMode.off()
  end

  log.info("thermostat_mode_report_handler - ready to set_field")
  device:set_field(MODE, mode, {persist = true})

  log.info("thermostat_mode_report_handler event: ", event)
...
local function device_init(self, device)
    ...
    -- local setCommand = Association:Set({
    --     grouping_identifier = 4,
    --     node_ids = {1}
        -- §groupingIdentifier = 4,
        -- nodeId = 1
    -- })
    -- device:send(setCommand)
    -- log.info("device_init - Association done")
end
...
log.info("secure_srt321_thermostat init.lua")

local thermostat = ZwaveDriver("secure_srt321_thermostat", secure_srt321_thermostat)
log.info("srt321thermostat init.lua")
thermostat:run()
